...
public class PlayerInventory : MonoBehaviourPun
{
    ...
    void Start()
    {
        Debug.Log("🟢 PlayerInventory Start: Initialisation...");
        LoadItemSprites();
    ...
    private void LoadItemSprites()
    {
        ...
        if (sprites.Length == 0)
        {
            Debug.LogError("❌ Aucun sprite trouvé dans Resources/InventorySprites !");
        }
    ...
    public bool RemoveItem(string itemName, float amount = 1f)
    {
        ...
        {
            Debug.LogWarning($"⚠️ Impossible de retirer {amount}x {itemName}, quantité insuffisante.");
            return false;
        }
        ...
        if (inventory[itemName] <= 0)
        {
            ...
            Debug.Log($"❌ {itemName} retiré de l'inventaire.");
        }
    ...
    public void ClearItem(string itemName)
    {
        if (inventory.ContainsKey(itemName))
        {
            inventory.Remove(itemName);
            inventoryKeys.Remove(itemName);
            Debug.Log($"🗑️ {itemName} supprimé de l'inventaire.");
        }
    }
    ...
    public void SwitchToNextItem()
    {
        ...
        {
            Debug.LogWarning("⚠️ Aucun objet dans l'inventaire !");
            return;
        }
